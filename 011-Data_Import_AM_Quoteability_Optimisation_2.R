...
RateAdjustment=read_excel('P:/Underwriting/Pricing Pipelines/IV-Home Pricing-Engine/Home MI/Home Quotes Conversion Risk Data DWH_Remove Renewal Confrimation with 1 ins.xlsx',sheet='Funnel')
RateAdjustment=RateAdjustment%>%filter(LoadingChangeIndicator==1)
RateAdjustment=RateAdjustment%>%mutate(Adjustment=`New Load`/Loading)
# Claims Data ####
                                                               ...
                                                               dplyr::if_else(QuoteDate<as.character('2024-01-13',format='%Y-%m-%d'),'4-GAP 1',
                                                                              dplyr::if_else(QuoteDate<as.character('2024-01-27',format='%Y-%m-%d'),'5-RateRed and Gap',
                                                                                             dplyr::if_else(QuoteDate<as.character('2024-01-31',format='%Y-%m-%d'),'6-Quoteability Changes',
                                                                                                            dplyr::if_else(QuoteDate<as.character('2024-02-16',format='%Y-%m-%d'),'7-QuoteArea Rate Change',
                                                                                                                           dplyr::if_else(QuoteDate<as.character('2024-02-22',format='%Y-%m-%d'),'8-Buying Behaviour','9-Rate adjust Inc+Dec')))))))))
Risk_Cover$InsuranceCompany='Accredited Insurances Limited(RE9090)'
Risk_Cover$Outcome='Premium Returned'
Quotes=merge(x=Quotes,y=Risk_Cover[,which(names(Risk_Cover)%in%c(RateAdjustment$Variable,"HomeRiskId","InsuranceCompany",'Outcome'))],by=c("HomeRiskId","InsuranceCompany",'Outcome'),all.x = TRUE)

# Adjust Quotes Rates ####
Quotes$NetPremiumAdjusted=Quotes$NetPremium
# tempQuotes=Quotes
# tempQuotes=merge(x=tempQuotes,y=Risk_Cover,by="HomeRiskId",all.x = TRUE)
...
#   Quotes$NetPremiumAdjusted[QuotesAffected]=Quotes$NetPremium[QuotesAffected]*RateAdjustment$Adjustment[f]
# }
# Quotes$NetPremium= Quotes$NetPremiumAdjusted
# Quotes=Quotes%>%mutate(NetPremium=dplyr::if_else(DatePeriod=='9-Rate adjust Inc+Dec',NetPremium*1.022,NetPremium))
# rm(tempQuotes)
...
# Risk Data ####
LatestEffectiveness=Quotes%>%
  # filter(DatePeriod%in%c('9-Rate adjust Inc+Dec','8-Buying Behaviour'))%>%
  select(QuoteDate,QuoteId,HomeRiskId,BusinessProcess,EventType,SchemeCode,InsuranceCompany,CoverEffectiveFrom,Outcome,TotalPayable,NetPremium,Levy,MinAltPrem,Rank,InsurersQuoting,NetPremiumAdjusted,DatePeriod)%>%
  mutate(HomeRiskId=toupper(HomeRiskId),
         ...
         IVEffectiveQuotesAdjust=dplyr::if_else(Outcome=='Premium Returned' & InsuranceCompany == "Accredited Insurances Limited(RE9090)" & NetPremiumAdjusted<=MinAltPrem,1,0),
         IVNetPremium=dplyr::if_else(Outcome=='Premium Returned' & InsuranceCompany == "Accredited Insurances Limited(RE9090)",NetPremium,NA),
         IVNetPremiumAdj=dplyr::if_else(Outcome=='Premium Returned' & InsuranceCompany == "Accredited Insurances Limited(RE9090)",NetPremiumAdjusted,NA),
  ...
  # filter(EventType!='New Business Policy Accepted')%>%
  group_by(HomeRiskId,BusinessProcess,EventType,QuoteDate,DatePeriod)%>%
  summarise(IVOutcome=first(IVOutcome),
            ...
            IVEffectiveQuotesAdj=sum(IVEffectiveQuotesAdjust,na.rm = T),
             IVNetPremium=mean(IVNetPremium,na.rm = T),
            IVNetPremiumAdj=mean(IVNetPremiumAdj,na.rm = T),
...
Risk_Cover$Outcome<-NULL
Risk_Cover$InsuranceCompany<-NULL
...
# Summary Effective ####
SummaryEffectiveness=LatestEffectiveness_Risk%>%group_by(DatePeriod)%>%
  filter(EventType%in%c('Quotation Provided'))%>%
  filter(!BusinessProcess%in%c('Mid-term Cancellation','Permanent Mid Term Adjustment'))%>%
  summarise(Volume=n(),
            IVNetPremiumWin=mean(dplyr::if_else(IVEffectiveQuotes==1,IVNetPremium,NA),na.rm = T),
            IVNetPremiumAdjWin=mean(dplyr::if_else(IVEffectiveQuotesAdj==1,IVNetPremiumAdj,NA),na.rm = T),
             IVQuotes=sum(IVQuotes,na.rm = T),
             IVRefDec=sum(IVRefDec,na.rm = T),
             ...
             IVEffectiveQuotes=sum(IVEffectiveQuotes,na.rm = T),
            IVEffectiveQuotesAdj=sum(IVEffectiveQuotesAdj,na.rm = T),
             Effectiveness=IVEffectiveQuotes/IVQuotes,
            IVEffectivenessAdj=IVEffectiveQuotesAdj/IVQuotes,
             IVNetPremium=mean(IVNetPremium,na.rm = T),
            IVNetPremiumAdj=mean(IVNetPremiumAdj,na.rm = T),
...
LatestEffectiveness_Risk_Long=LatestEffectiveness_Risk_Long%>%group_by(Variable,Value)%>%mutate(IVQuotes=as.numeric(IVQuotes),
...
QuoteabilityFunnelLong=LatestEffectiveness_Risk%>%
  filter(BusinessProcess%in%c('Renewal Confirmation','New Business'))%>%
  filter(DatePeriod%in%c('9-Rate adjust Inc+Dec'))%>%
  filter(dplyr::if_else(InsurersQuoting==1 & BusinessProcess=='Renewal Confirmation',1,0)!=1)%>%
  mutate(OpenClaim=dplyr::if_else(is.na(OpenClaim),0,OpenClaim),
         ClaimsCount=dplyr::if_else(is.na(ClaimsCount),0,ClaimsCount))%>%
  mutate(
