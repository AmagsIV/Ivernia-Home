...
to_keep <- c("FirstTimeBuyer","BirthDate","MaritalStatus","OccupationType","EmploymentType",
             "num_ph","smoker_present","HomeRiskId","Age")
dt_polholder <- dt_polholder[,to_keep]

dt_quote$QuoteDate=dt_quote$`$ExtractDateTime`
to_drop <- c("DataSource","$ExtractDateTime","InsuranceCompany","SchemeOutcome","TotalPayable","QuoteReference","OverrideAmount",
             "OverrideProvider","OverrideAuthCode","Levy","Commission")#"SchemeCode",
...
dt_quote <- dt_quote[,!(names(dt_quote)%in%to_drop)]
dt_quote$QuoteDate=as.Date(dt_quote$QuoteDate)
gc()
dt_quote <- dt_quote[dt_quote$Outcome=="Premium Returned",]
# dt_quote <- dt_quote[dt_quote$NetPremium>0,]
dt_quote=dt_quote%>%filter(QuoteDate>=as.Date('2023-12-01', format = '%Y-%m-%d'))%>%arrange(desc(QuoteDate))
dt_quote_min_2 <-dt_quote %>%
  group_by(HomeRiskId,CoverEffectiveFrom,SchemeCode) %>%
  summarise(min_NetPremium = min(NetPremium))
dt_quote_min_2$MinSchemeCode=dt_quote_min_2$SchemeCode
dt_quote_min_2$SchemeCode=NULL
dt_quote_min <- aggregate(dt_quote$NetPremium,by=list(HomeRiskId=dt_quote$HomeRiskId,CoverEffectiveFrom=dt_quote$CoverEffectiveFrom),FUN=function(x) min(x))
...
dt_quote_mean=dt_quote%>%group_by(HomeRiskId)%>%
  arrange(NetPremium)%>%
  mutate(rowid=row_number())%>%
  filter(rowid<=10)%>%
  summarise(avr_prem=mean(NetPremium))%>%select(HomeRiskId,avr_prem)
dt_quote_mean_allQuotes <- aggregate(dt_quote$NetPremium,by=list(HomeRiskId=dt_quote$HomeRiskId),FUN=function(x) mean(x))
colnames(dt_quote_mean_allQuotes)[colnames(dt_quote_mean)=="x"] <- "avr_prem_AllQuotes"
dt_quote_count <- aggregate(dt_quote$NetPremium,by=list(HomeRiskId=dt_quote$HomeRiskId),FUN=function(x) length(x))
colnames(dt_quote_count)[colnames(dt_quote_count)=="x"] <- "qcount"
# dt_quote <- merge(x=dt_quote[,c("HomeRiskId","CoverEffectiveFrom")],y=dt_quote_min)
...
dt <- merge(x=dt,y=dt_quote_mean_allQuotes,by="HomeRiskId")
